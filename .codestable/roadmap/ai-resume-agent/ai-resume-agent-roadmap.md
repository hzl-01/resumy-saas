...
当前状态更新：主链路 `上传文件 -> JD -> AI 草稿 -> editor -> PDF -> 浏览器下载` 已经能跑通；本 roadmap 剩余工作的重点，不再是“把链路连起来”，而是**把仓库原有 `agent-resume` 的剩余脑子完整产品化**，并补齐对应的 CodeStable feature 闭环。
...
3. **ai-document-intake** — 支持 pdf/docx/text 输入并交给 AI service 做统一 intake
   ...
   - 状态：done
   - 对应 feature：2026-05-13-ai-document-intake
   - 备注：替代纯启发式 import 方案
...
4. **ai-resume-structuring** — 实现 AI 将原始材料结构化为 ResumeDocument
   ...
   - 状态：done
   - 对应 feature：2026-05-13-ai-resume-structuring
...
5. **ai-schema-guard** — 在 Bun 侧落地 AI 输出校验、warnings 与错误收口
   ...
   - 状态：done
   - 对应 feature：2026-05-13-ai-schema-guard
...
7. **ai-clarification-loop** — AI 缺信息时向用户追问，用户回答后继续生成
   ...
   - 状态：done
   - 对应 feature：2026-05-13-ai-clarification-loop
...
8. **ai-result-apply** — AI 生成结果自动落成 draft resume，并跳转到现有编辑器
   ...
   - 状态：done
   - 对应 feature：2026-05-13-ai-result-apply
...
9. **ai-jd-tailor** — 输入 JD，对已有简历生成定制版 ResumeDocument
   ...
   - 状态：done
   - 对应 feature：2026-05-13-ai-jd-tailor
...
当前收尾顺序更新：既然主链路已通，接下来优先级变成：

1. 把已落地但尚未完整补档的 feature（`ai-document-intake`、`ai-result-apply`）补齐 CodeStable 闭环
