...
resumy 最初是基于 Bun + TypeScript 的简历 PDF 生成 CLI，通过结构化命令行参数 + 内置模板生成 PDF 简历。当前仓库已扩展出 Web 服务，并以 AI 主入口为产品默认路径：Bun 主服务继续承载用户、草稿、模板和 PDF 输出，Go + Eino sidecar 负责 AI workflow / agent 编排与直出草稿。
...
- **AI sidecar service** — 运行在 `services/ai/` 的独立 Go 进程，默认监听 `127.0.0.1:8081`，承载 Eino / OpenAI-compatible 驱动的 Internal AI API
...
- AI 主入口按“旧简历/背景优先、JD 第二、再继续生成”的状态机建模；无 JD 时 Bun 主服务直接进入 `needs_input`，而不是立刻把请求透传给 sidecar
